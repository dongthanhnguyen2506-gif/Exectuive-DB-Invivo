...
Secrets được inject qua environment variables — không hardcode password
"""
...
def clean_row(row):
    """Bỏ prefix 'TableName[ColumnName]' → 'ColumnName'"""
...
def build_summary(rows):
    ...
    # Top partners
    top_partners = sorted(
        [{"partnerId":k, "partnerCode":v["name"],
          "revenue":round(v["revenue"]), "listed":round(v["listed"]),
          "orders":len(v["orders"])}
    ...
    # Service mix
    service_mix = sorted(
        [{"group":k, "revenue":round(v["revenue"]),
          "listed":round(v["listed"]), "count":v["count"]}
